...
def _get_label_image(text, font_color_tuple_bgr= (0,0,0), background_color_tuple_bgr=(255,255,255)):
...
def add(image, left, top, right, bottom, label=None, color=None):
    ...
    colors = [_rgb_to_bgr(item) for item in (color, complement(*color))]
    ...
    if label:
        ...
        label_image =  _get_label_image(label)
